...
const CitizenLogin = () => {
  ...
  const stateDistricts = {
    ...
    'rajasthan': [
      ...
      'Jaipur', 'Jaisalmer', 'Jalore', 'Jhalawar', 'Jhunjhunun', 'Jodhpur', 'Karauli', 
  ...
  const handleSubmit = async (e) => {
    ...
    if (isRegistering) {
      // Registration logic
      if (!formData.name || !formData.state || !formData.district || !formData.contact || !formData.password || !formData.confirmPassword) {
      ...
      try {
        ...
        if (result.success) {
          ...
          setTimeout(() => {
            navigate('/citizen/file-complaint');
          }, 2000);
        } else {
      ...
      // Login logic
      if (!formData.state || !formData.district || !formData.contact || !formData.password) {
        setMessage('Please fill in all required fields.');
        return;
      }
      ...
      try {
        ...
        if (result.success) {
          ...
          setMessage(result.error || 'Login unsuccessful. Please verify your details and try again.');
        }
      } catch (error) {
        console.error('Login error:', error);
        setMessage('Login unsuccessful. Please verify your details and try again, or go back to login.');
      }
    }
  };
  ...
  return (
    <Container>
      <LoginCard>
        ...
        <Title>{isRegistering ? 'Citizen Registration' : 'Citizen Login'}</Title>
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <FormGroup>
            <Label htmlFor="password">{isRegistering ? 'Password' : 'OTP / Password'}</Label>
            <Input
              type="password"
              id="password"
              name="password"
              placeholder={isRegistering ? 'Enter Password' : 'Enter OTP or Password'}
          ...
          <LoginButton type="submit">
            {isRegistering ? 'Register & Login' : 'Login to Citizen Portal'}
          </LoginButton>
        </Form>
