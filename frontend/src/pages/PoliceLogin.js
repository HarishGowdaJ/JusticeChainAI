...
const PoliceLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isRegistering) {
      // Registration logic
      if (!formData.name || !formData.email || !formData.state || !formData.district || !formData.area || !formData.stationName || !formData.badgeNumber || !formData.password || !formData.confirmPassword) {
      ...
      try {
        ...
        const result = await register({
          ...
          role: 'police',
          phone: formData.badgeNumber,
          state: formData.state,
          district: formData.district,
          area: formData.area,
          stationName: formData.stationName,
          policeStation: formData.stationName, // Use stationName for policeStation field
    ...
    try {
      ...
      if (result.success) {
        ...
        setMessage(result.error || 'Invalid Police ID or Password. Please check your credentials and try again.');
      }
    } catch (error) {
      console.error('Login error:', error);
      setMessage('Invalid Police ID or Password. Please check your credentials and try again, or go back to login.');
      }
    }
  };
  ...
  return (
    <Container>
      <LoginCard>
        ...
        <Title>{isRegistering ? 'Police Registration' : 'Police Login'}</Title>
        
        <ToggleButton onClick={() => setIsRegistering(!isRegistering)}>
          {isRegistering ? 'Already have an account? Login' : 'New officer? Register here'}
        </ToggleButton>
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <FormGroup>
            <Label htmlFor="badgeNumber">Police ID / Badge Number</Label>
            <Input
              type="text"
              id="badgeNumber"
              name="badgeNumber"
          ...
          <LoginButton type="submit">
            {isRegistering ? 'Register & Login' : 'Login to Police Portal'}
          </LoginButton>
        </Form>
