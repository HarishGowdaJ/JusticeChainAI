  ...
  try {
    ...
    router.post('/', auth, upload.array('documents', 10), [
      ...
      try {
        ...
        const caseFile = new CaseFile({
          ...
          firId,
          complaintId: fir.complaintId,
          courtName: user.role === 'court' ? user.courtName : 'District Court',
          judgeName,
          publicProsecutor,
          caseType,
          caseDetails,
        ...
        // Blockchain sync - create case on chain
        try {
          await createCaseOnChain(caseNumber);
        } catch (err) {
          console.error('Blockchain create case failed:', err);
        }
    ...
    router.put('/:id/status', auth, [
      ...
      try {
        ...
        // Blockchain update
        try {
          await updateCaseStatusOnChain(caseFile.caseNumber, status);
        } catch (err) {
          console.error('Blockchain update case status failed:', err);
        }
    ...
    router.post('/:id/hearings', auth, [
      ...
      try {
        ...
        // Blockchain update
        try {
          await scheduleHearingOnChain(caseFile.caseNumber, Math.floor(new Date(date).getTime() / 1000), purpose, notes);
        } catch (err) {
          console.error('Blockchain schedule hearing failed:', err);
        }
    ...
    router.post('/:id/judgment', auth, [
      body('verdict').isIn(['guilty', 'not_guilty', 'acquitted', 'convicted']).withMessage('Invalid verdict'),
      ...
      try {
        ...
        // Blockchain update
        try {
          await recordJudgmentOnChain(caseFile.caseNumber, verdict, sentence, fine, judgmentText);
        } catch (err) {
          console.error('Blockchain record judgment failed:', err);
        }
